...
reference <- LoadReference(path = "/reference-data/human_liver/")

# Load the query object for mapping
# Change the file path based on where the query file is located on your system.
query <- LoadFileInput(path = "hg38_filtered_feature_bc_matrix.h5")
...
# you set in the app
cells.use <- query[["nCount_RNA", drop = TRUE]] <= 8262 &
  query[["nCount_RNA", drop = TRUE]] >= 500 &
  query[["nFeature_RNA", drop = TRUE]] <= 4378 &
  query[["nFeature_RNA", drop = TRUE]] >= 58
...
names(x = refdata) <- c("\"celltype.l2\"", "\"celltype.l1\"")
if (FALSE) {
...
id <- c("\"celltype.l2\"", "\"celltype.l1\"")[1]
